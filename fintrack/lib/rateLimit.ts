...
function buildStore(): RateLimitStore {
  ...
  if (process.env.NODE_ENV === 'production' && (!redisUrl || !redisToken)) {
    // Loud warning: in-memory counters reset on cold-starts and are NOT shared
    // across serverless instances. Set UPSTASH_REDIS_REST_URL and
    // UPSTASH_REDIS_REST_TOKEN in Vercel env vars before going live.
    console.error(
